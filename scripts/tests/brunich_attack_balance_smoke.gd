...
func _run() -> void:
	...
	_expect(is_equal_approx(float(spread_weapon.ShootInterval), 0.2294117647), "la escopeta enemiga debe bajar su cadencia en 15 por ciento")
	_expect(is_equal_approx(float(spread_weapon.PROJECTILE_SPEED), 410.55), "la escopeta enemiga debe lanzar pellets 15 por ciento mas lentos")
	_expect(is_equal_approx(float(spread_weapon.SPREAD_TOTAL), 0.38675), "la escopeta enemiga debe cerrar su patron otro 15 por ciento")
	_expect(int(spread_live_profile.get("damage", -1)) == 4, "la escopeta enemiga debe subir su dano por pellet al siguiente entero util")
	_expect(is_equal_approx(float(spread_weapon.PROJECTILE_SPEED) * float(spread_live_profile.get("life_time", 0.0)), 431.0775), "la escopeta enemiga debe alcanzar 15 por ciento menos distancia total")
	...
	_expect(int(spread_pickup_profile.get("projectile_profile", {}).get("damage", -1)) == 4, "la escopeta robada debe mantener el nuevo dano por pellet")
	...
	_expect(is_equal_approx(float(pierce_weapon.PROJECTILE_SPEED), 2406.95), "el proyectil sniper enemigo debe viajar mucho mas rapido en esta iteracion")
	...
	_expect(is_equal_approx(float(pierce_pickup_profile.get("projectile_speed", -1)), 2406.95), "el pierce robado debe conservar la nueva velocidad del proyectil")
	...
	_expect(is_equal_approx(float(ai_core_weapon.ShootInterval), 0.6), "el AI core debe recargarse al mismo ritmo que vuelve el dash base sin mejoras")
	...
	_expect(int(ai_core_weapon._get_beam_profile().get("damage_per_tick", -1)) <= 26, "el beam del AI core debe reflejar la bajada de dano de esta iteracion")
	...
	_expect(player.DashCharges == 0, "el dash base debe gastar su unica carga al activarse")
	await _wait_physics_frames(20)
	_expect(player.DashCharges == 0, "el dash base no debe regresar antes de acercarse a 0.6 segundos")
	await _wait_physics_frames(12)
	_expect(player.DashCharges == 1, "el dash base debe recargarse cerca de 0.6 segundos sin mejoras")
